...
class EmployerApi
{
     ...
     * Create an Employer
     *
     * @param  \SynergiTech\Staffology\Model\Employer|null $employer employer (optional)
     ...
     * @throws ApiException on non-2xx response or if the response body is not in the expected format
     * @throws InvalidArgumentException
     * @return \SynergiTech\Staffology\Model\Employer
     */
    public function createEmployer(
        ?\SynergiTech\Staffology\Model\Employer $employer = null,
        string $contentType = self::contentTypes['createEmployer'][0]
    ): \SynergiTech\Staffology\Model\Employer
    {
        list($response) = $this->createEmployerWithHttpInfo($employer, $contentType);
        return $response;
    }
     ...
     * Create an Employer
     *
     * @param  \SynergiTech\Staffology\Model\Employer|null $employer (optional)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['createEmployer'] to see the possible values for this operation
     *
     * @throws ApiException on non-2xx response or if the response body is not in the expected format
     * @throws InvalidArgumentException
     * @return array of \SynergiTech\Staffology\Model\Employer, HTTP status code, HTTP response headers (array of strings)
     */
    ...
    {
        $request = $this->createEmployerRequest($employer, $contentType);
        ...
        try {
            ...
            switch($statusCode) {
                case 201:
                    if (in_array('\SynergiTech\Staffology\Model\Employer', ['\SplFileObject', '\Psr\Http\Message\StreamInterface'])) {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\SynergiTech\Staffology\Model\Employer' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\SynergiTech\Staffology\Model\Employer', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
            ...
            $returnType = '\SynergiTech\Staffology\Model\Employer';
            if (in_array($returnType, ['\SplFileObject', '\Psr\Http\Message\StreamInterface'])) {
                $content = $response->getBody(); //stream goes to serializer
            } else {
                $content = (string) $response->getBody();
    ...
    {
        return $this->createEmployerAsyncWithHttpInfo($employer, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    ): Request
    {
        ...
        if (isset($employer)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($employer));
            } else {
                $httpBody = $employer;
            }
        } elseif (count($formParams) > 0) {
    ...
    {
        list($response) = $this->indexEmployerWithHttpInfo($employerGroupCode, $contentType);
        return $response;
    }
    ...
    {
        $request = $this->indexEmployerRequest($employerGroupCode, $contentType);
    ...
    {
        return $this->indexEmployerAsyncWithHttpInfo($employerGroupCode, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    {
        list($response) = $this->updateEmployerWithHttpInfo($id, $employer, $contentType);
        return $response;
    }
    ...
    {
        $request = $this->updateEmployerRequest($id, $employer, $contentType);
    ...
    {
        return $this->updateEmployerAsyncWithHttpInfo($id, $employer, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
